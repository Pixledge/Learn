...
public class LevelManager : MonoBehaviour
{
    ...
    public Flags[] ingrTarget = new Flags[2];
   ...
   //     SoundBase.Instance.audio.PlayOneShot(SoundBase.Instance.getStarIngr);
    ...
    public void CheckWinLose()
    {
        if (Limit <= 0)
        {
            ...
            else if (LevelManager.THIS.target == Target.FLAG && (LevelManager.THIS.ingrCountTarget[0] > 0 || LevelManager.THIS.ingrCountTarget[1] > 0))
            {
                lose = true;
            }
            ...
            else if (LevelManager.Score >= LevelManager.THIS.star1 && LevelManager.THIS.target == Target.FLAG && (LevelManager.THIS.ingrCountTarget[0] <= 0 && LevelManager.THIS.ingrCountTarget[1] <= 0))
            {
                gameStatus = GameState.PreWinAnimations;
 ...
 //           GenerateNewItems();
    ...
    void AnimateField(Vector3 pos)
    {
        ...
        float yOffset = 0;
        if (target == Target.FLAG) yOffset = 0.3f;
    ...
    void ProcessGameDataFromString(string mapText)
    {
        ...
        foreach (string line in lines)
        {
            ...
            else if (line.StartsWith("COLLECT ITEMS "))
            {
                ...
                {
                    if (target == Target.FLAG)
                        ingrTarget[i] = (Flags)int.Parse(blocksNumbers[i]);
                    else if (target == Target.COLLECT)
