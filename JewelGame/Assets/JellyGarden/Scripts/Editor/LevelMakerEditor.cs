...
public class LevelMakerEditor : EditorWindow
{
    ...
    private Flags[] ingr = new Flags[2];
    ...
    void GUITarget()
    {
        ...
        else if (target == Target.FLAG)
        {
            ...
            {
                GUILayout.BeginHorizontal();
                Flags oldIngr = ingr[i];
                int oldCount = ingrCount[i];
                ingr[i] = (Flags)EditorGUILayout.EnumPopup(ingr[i], GUILayout.Width(100));
                if (i == 0 && ingr[i] == Flags.Flag2) ingr[i] = Flags.Flag1;
                else if (i == 1 && ingr[i] == Flags.Flag1) ingr[i] = Flags.Flag2;
                if ((int)ingr[i] > 0)
                    ingrCount[i] = EditorGUILayout.IntField("", ingrCount[i], new GUILayoutOption[] { GUILayout.Width(100) });
                else
                    ingrCount[i] = 0;
    ...
    void ProcessGameDataFromString(string mapText)
    {
        ...
        foreach (string line in lines)
        {
            ...
            else if (line.StartsWith("COLLECT ITEMS "))
            {
                ...
                {
                    if (target == Target.FLAG)
                        ingr[i] = (Flags)int.Parse(blocksNumbers[i]);
                    else if (target == Target.COLLECT)
